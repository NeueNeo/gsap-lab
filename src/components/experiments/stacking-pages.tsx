...
export function StackingPages({ onReplay }: Props) {
  void onReplay;
  ...
  useGSAP(
    () => {
      ...
      const bar = scroller.querySelector(".stack-progress-bar") as HTMLElement;
      const numPages = 4;
      if (bar) {
        const update = () => {
      ...
      const intro = scroller.querySelector(".stack-intro") as HTMLElement;
      if (intro) {
  ...
  return (
    <div ref={containerRef} className="h-full overflow-y-auto bg-zinc-950">
      <div className="stack-progress-bar fixed top-0 left-0 w-full h-1 bg-emerald-400 origin-left z-50" style={{ transform: "scaleX(0)" }} />
      ...
      {PAGES.map((page, i) => (
        <div
          key={i}
          className="stack-page sticky top-0"
